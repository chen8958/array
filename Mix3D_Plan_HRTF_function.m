function [P_half SorPos SorLen p]=Mix3D_Plan_HRTF_function(MicPos,SorPos)
c=343;  % sound speed
[N M]=size(MicPos);
...
for FrameNo=1:NumOfFrame
    ...
    for ff=1:length(Freqs)
        ...
        for ss = 1:SorNum
            ...
            kappa = [sind(SorPos(ss,1))*sind(SorPos(ss,2)) cosd(SorPos(ss,1))*sind(SorPos(ss,2)) cosd(SorPos(ss,2))];
