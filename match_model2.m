...
for i=1:MicNum
y(1,:)=y(1,:)+conv(p_source(i,:),reshape(H_filter_transpose(1,i,:),[1 length(H_filter)]));
y(2,:)=y(2,:)+conv(p_source(i,:),reshape(H_filter_transpose(2,i,:),[1 length(H_filter)]));
end
